...
gset <- getGEO("GSE100052", GSEMatrix =TRUE, AnnotGPL=TRUE)
...
gsms <- "000000111111222222888888333333444444555555666666777777"
...
gs <- factor(sml)
groups <- make.names(c("E7dot5","E8dot5","E9dot5","E11dot5","E12dot5","E14dot5","E16dot5","E18dot5","E10dot5"))
levels(gs) <- groups
gset$group <- gs
...
tT <- subset(tT, select=c("ID","adj.P.Val"))
...
par(mar=c(7,4,2,1))
title <- paste ("GSE100052", "/", annotation(gset), sep ="")
...
plotSA(fit2, main="Mean variance trend, GSE100052")
